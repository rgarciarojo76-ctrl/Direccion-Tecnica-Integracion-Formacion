 ...
 * SynergyGroup Card
 */
const SynergyGroup = React.memo(({ item, unifiedAs, onUnify, onUndoUnify }) => {
  ...
  const renderMessage = () => {
    if (scenario === 'reference_potential') {
      return (
        <div className="flex items-center gap-3 mb-4 px-2">
          <div className="synergy-icon-circle synergy-icon-ref-potential"><MapPin size={18} /></div>
          <div className="synergy-text-ref-potential">{item.suggestion}</div>
        </div>
      );
    }
    if (scenario === 'reference_unlikely') {
      return (
        <div className="flex items-center gap-3 mb-4 px-2">
          <div className="synergy-icon-circle synergy-icon-ref-unlikely"><AlertTriangle size={18} /></div>
          <div className="synergy-text-ref-unlikely">{item.suggestion}</div>
        </div>
      );
    }
    if (scenario === 'overflow') {
      return (
        <div className="flex items-center gap-3 mb-4 px-2">
          <div className="synergy-icon-circle synergy-icon-warning"><AlertTriangle size={18} /></div>
          <div className="synergy-text-warning">{item.suggestion}</div>
        </div>
      );
    }
    return (
      <div className="flex items-center gap-3 mb-4 px-2">
        <div className="synergy-icon-circle"><Users size={18} /></div>
        <div className="synergy-text-primary">{item.suggestion}</div>
      </div>
    );
  };
        ...
        <td colSpan="10" className="p-0 border-none">
          <div className={cardClass}>
            ...
            <div className="synergy-card-badges-container">
              <div className={badge.className}><span>{badge.text}</span></div>
              {unifiedAs && (
            ...
            <div className="p-6 pt-10">
              ...
              {isUnifiable && (
                <div className="unify-actions">
                  {!unifiedAs ? (
                    <>
                      <button className="btn-unify btn-unify-aspy" onClick={() => onUnify && onUnify(item.id, 'ASPY')}>Imparte ASPY</button>
                      <button className="btn-unify btn-unify-mas" onClick={() => onUnify && onUnify(item.id, 'MAS')}>Imparte MAS</button>
                    </>
                  ) : (
                    <button className="btn-undo-unify" onClick={() => onUndoUnify && onUndoUnify(item.id)}>
                      <RotateCcw size={14} /> Deshacer unificación
                    </button>
                  )}
                </div>
              )}
